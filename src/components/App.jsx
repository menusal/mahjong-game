...
import { areTilesMatch, checkForMoves, findValidPairs, updateSelectable } from '../utils/gameUtils.js';
import { GAME_STATES } from '../constants/uiConstants.js';
import GameBoard from './GameBoard.jsx';
import GameControls from './GameControls.jsx';
import GameOverlay from './GameOverlay.jsx';
import MotivationalWord, { getRandomMotivationalWord } from './MotivationalWord.jsx';
...
const App = () => {
  ...
  }, [board, selectedId, gameState, moveHistory, playClickSound, playMatchSound, playWinSound, playErrorSound, playShuffleSound, setBoard, setSelectedId, setGameState, setMoveHistory, setHint, handleShuffleRemaining, triggerConfetti, triggerWinConfetti]);
  ...
  const handleUndo = useCallback(() => {
    ...
    setHint(null);
  }, [board, moveHistory, playUndoSound, setBoard, setMoveHistory, setSelectedId, setGameState, setHint]);
  ...
  const handleStartGameWithAudio = useCallback(() => {
    startAudioContext();
    handleStartGame();
  ...
  const gameWorldClasses = `
    ...
    ${(gameState === GAME_STATES.START || gameState === GAME_STATES.WON || 
       gameState === GAME_STATES.STUCK || gameState === GAME_STATES.SHUFFLING) 
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-800 text-white p-2 font-sans overflow-hidden relative">
      ...
      <div className={gameWorldClasses}>
        ...
        <div className="mt-4 text-base text-gray-400">
          Nivel: {level} | Fichas restantes: {board.filter(t => !t.isMatched).length}
        </div>
      </div>
    </div>
  );
